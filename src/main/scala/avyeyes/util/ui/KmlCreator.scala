...
class KmlCreator {
	def createCompositeKml(avalanches: List[Avalanche]): Elem = {
	    <kml xmlns="http://www.opengis.net/kml/2.2" xmlns:gx="http://www.google.com/kml/ext/2.2" 
	        xmlns:kml="http://www.opengis.net/kml/2.2" xmlns:atom="http://www.w3.org/2005/Atom">
	        <Document>{for(a <- avalanches) yield createPlacemark(a)}</Document></kml>
	}
	
	private def createPlacemark(avalanche: Avalanche): Elem = {
	  <Placemark>
	...
	private def getHtmlDesc(avalanche: Avalanche): Elem = {
	        <table>
	...
	private def getPolygon(avalanche: Avalanche): Elem = 
