...
class AvalancheDaoSelectTest extends Specification with InMemoryDB with AvalancheHelpers {
  ...
  "Ordering" >> {
    ...
    "Selects can be ordered by date ascending" >> {
      ...
      val dateAscOrderQuery = AvalancheQuery(order = List((OrderField.avyDate, OrderDirection.Asc)))
    ...
    "Selects can be ordered by date descending" >> {
      ...
      val dateDescOrderQuery = AvalancheQuery(order = List((OrderField.avyDate, OrderDirection.Desc)))
